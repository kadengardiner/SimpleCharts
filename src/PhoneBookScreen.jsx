...
export default function PhoneBookScreen({ uid }) {
  ...
  useEffect(() => {
    async function load() {
      const snap = await getDocs(collection(db, 'users', uid, 'phonebook'))
  ...
  async function addEntry() {
    ...
    const ref = await addDoc(collection(db, 'users', uid, 'phonebook'), {
      label: newLabel.trim(),
      value: newValue.trim(),
    })
  ...
  async function deleteEntry(id) {
    await deleteDoc(doc(db, 'users', uid, 'phonebook', id))
